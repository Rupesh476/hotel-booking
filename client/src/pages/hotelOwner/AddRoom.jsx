...
const[inputs,setInputs] = useState({
  ...
  return (
    <form>
      ...
      <div className='grid grid-cols-2 sm:flex gap-4 my-2 flex-wrap'>
        {Object.keys(images).map((key)=>(
          <label htmlFor={`roomImage${key}`} key={key}>
            ...
            src={images[key] ? URL.createObjectURL(images[key]) : assets.uploadArea} alt="upload-image icon" />
      ...
      <div className='w-full flex msx-sm:flex-col sm:gap-4 mt-4'>
        <div className='flex-1 max-w-48 '>
          <p className='text-gray-800 mt-4'>Room Type</p>
          
          <select value={inputs.roomType} onChange={e=> setInputs({...inputs, roomType: e.target.value})} 
          className='border opacity-70 border-gray-300 mt-1 w-full'>
      ...
      <div>
        ...
        <input type="number" placeholder='0' value={inputs.pricePerNight} onChange={e=> setInputs({...inputs, pricePerNight: e.target.value})}
